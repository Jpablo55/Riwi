...
def generate_reports(library):
    ...
    if chose_option == "1":
        ...
        print("Mostrar el libro más antiguo y el más reciente por género.")
        
        libros_por_genero = {}

        for titulo, datos in library.items():
            genero = datos["genre"]
            anio = datos["year"]

            # Si el género no está en el diccionario, lo agregamos
            if genero not in libros_por_genero:
                libros_por_genero[genero] = []

            # Guardamos el título y el año en la lista del género
            libros_por_genero[genero].append((titulo, anio))

        # Paso 2: Buscar el libro más antiguo y el más reciente por género
        print("Reporte de libros por género:\n")

        for genero, lista_libros in libros_por_genero.items():
            # Inicializamos con el primer libro
            mas_antiguo = lista_libros[0]
            mas_reciente = lista_libros[0]

            # Recorremos la lista para comparar años
            for libro in lista_libros:
                if libro[1] < mas_antiguo[1]:
                    mas_antiguo = libro
                if libro[1] > mas_reciente[1]:
                    mas_reciente = libro

            # Mostramos el resultado
            print("Género:", genero)
            print("  Libro más antiguo:", mas_antiguo[0], "(", mas_antiguo[1], ")")
            print("  Libro más reciente:", mas_reciente[0], "(", mas_reciente[1], ")\n")
